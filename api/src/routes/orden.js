...
const idProduct = req.params.idproducto;
  ...
  .then(orden => {
    // Creamos el carrito y agregamos el producto
...
server.put('/:id/:cantidad', function(req, res) {
  ...
  .then(() => { 
    return res.send("Se agrego mas unidades al carrito")
  })
  .catch(() => {
    return res.status(400).send('No hay mas unidades de este articulo!')
  })
});
